...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	// Register class and create window
	{
		...
		HWND hwnd = CreateWindow(L"DirectXTKModelViewerWindowClass", L"Displacement Mapping (DirectX 11)", WS_OVERLAPPEDWINDOW,
	...
	case WM_USER:
		if (game)
		{
			...
			else if (lParam == 1 || lParam == 2 || lParam == 3 || lParam == 4)
			{
				...
				ofn.lpstrFile[0] = 0;
				ofn.nMaxFile = MAX_PATH;
				ofn.lpstrFilter = L"DirectDraw Surface\0*.dds\0";
				ofn.nFilterIndex = s_filterIndex;
				ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;
				if (GetOpenFileName(&ofn))
				{
